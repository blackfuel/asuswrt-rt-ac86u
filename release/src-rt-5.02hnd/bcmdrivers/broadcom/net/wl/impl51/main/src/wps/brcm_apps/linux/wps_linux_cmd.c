 ...
 *
 * $Id: wps_linux_cmd.c 444957 2013-12-23 09:15:49Z $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
wps_configap(char *ifname)
{
	...
	uilen += snprintf(uibuf + uilen, IFNAMSIZ+ sizeof("wps_ifname= "),
		"wps_ifname=%s ", ifname);
	set_wps_env(uibuf);
	return;
}
	...
	if (!strcmp(method, "pbc")) {
		...
		if (autho_sta_mac) {
			if (wps_hwaddr_check(autho_sta_mac) != 0)
				return -1;
			uilen += snprintf(uibuf + uilen,
				sizeof("wps_autho_sta_mac= ") + sizeof("00:11:22:33:44:55"),
