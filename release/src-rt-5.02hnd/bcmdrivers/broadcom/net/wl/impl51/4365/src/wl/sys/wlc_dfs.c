...
wlc_dfs_bg_upgrade_wlc(wlc_info_t *wlc)
{
	...
	FOREACH_UP_AP(wlc, idx, bsscfg)
	{
		...
		new_oper_mode = DOT11_OPER_MODE(0,
			WLC_BITSCNT(wlc->stf->valid_txchain_mask), bw);
...
wlc_dfs_bg_downgrade_wlc(wlc_info_t *wlc)
{
	...
	FOREACH_UP_AP(wlc, idx, bsscfg)
	{
		...
		new_oper_mode = DOT11_OPER_MODE(0, WLC_BITSCNT(wlc->stf->valid_txchain_mask)-1, bw);
		err = wlc_modesw_handle_oper_mode_notif_request(wlc->modesw, bsscfg,
				new_oper_mode, TRUE, MODESW_CTRL_OPMODE_IE_REQD_OVERRIDE);
		if (err != BCME_OK) {
			WL_DFS(("wl%d: failed to request modesw %d\n", wlc->pub->unit, err));
...
{
	wlc_info_t* wlc = dfs->wlc;
	bool is_edcrs_eu, is_us, is_ca, tdwr = FALSE, has_subband_info = DFS_HAS_SUBBAND_INFO(wlc);
	...
	is_edcrs_eu = wlc_is_edcrs_eu(wlc);
...
#endif /* WL11AC_80P80 */
	uint32 rand_tsf = R_REG(wlc->osh, &wlc->regs->u.d11regs.tsf_random);
	int16 rand_idx;
#if defined(BCMDBG)
...
wlc_dfs_chanspec(wlc_dfs_info_t *dfs, bool radar_detected)
{
	...
	if (chspec == 0) {
		...
		chspec = wlc_dfs_chanspec_rand(dfs, wlc_dfs_bw_list, wlc_dfs_bw_list_len,
				radar_detected);
	}
