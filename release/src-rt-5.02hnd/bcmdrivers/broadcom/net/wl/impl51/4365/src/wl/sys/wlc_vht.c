...
wlc_write_vht_op_ie(wlc_bsscfg_t *cfg, vht_op_ie_t *op_ie)
{
	...
	if (CHSPEC_IS80(chspec)) {
		...
		/* This is needed for WFA plugfest.
		 * For AP operating at 36/160, we announce Channel Center Frequency Segment 0 as 42
		 * and Channel Center Frequency Segment 1 as 50 and set channel width to 80Mhz.
		 * When a 160Mhz STA parses non-zero value for Channel Center Frequency Segment 1,
		 * it understands that the AP is 160Mhz capable.
		 */
		width = VHT_OP_CHAN_WIDTH_80;
...
#endif /* WL11ULB */
{
	...
	if (width == VHT_OP_CHAN_WIDTH_20_40) {
		...
		/*
		 * Channel Center Frequency Segment for 160Mhz is 50 or 114.
		 * If the Channel Center Frequency Segment 1 is 50 or 114,
		 * it means that the AP is 160Mhz.
		 */
		if ((chan2 == 50) || (chan2 == 114)) {
			chanspec = wf_channel2chspec(ht_primary, WL_CHANSPEC_BW_160);

			if (chanspec == INVCHANSPEC) {
				WL_TMP(("wl%d: %s: unexpected VHT 160 for channel %d ",
					vhti->wlc->pub->unit, __FUNCTION__, ht_primary));
			}
		} else {
	...
	ht_cap_ie_t *cap_ie, vht_cap_ie_t *vht_cap_ie, vht_op_ie_t *vht_op_ie, uint8 vht_ratemask)
{
	...
	if ((vht_op_ie && vht_op_ie->chan_width == VHT_OP_CHAN_WIDTH_160) ||
		(vht_cap_ie &&
		(vht_cap_ie->vht_cap_info & VHT_CAP_INFO_SUPP_CHAN_WIDTH_MASK) ==
		VHT_CAP_CHAN_WIDTH_SUPPORT_160)) {
	...
	*/
	...
	if (TXBF_ENAB(wlc->pub)) {
		...
		if (vht_cap_ie->vht_cap_info & VHT_CAP_INFO_MU_BEAMFMEE) {
			...
			if (VHT_MCS_MAP_GET_MCS_PER_SS((nrx + 1), scb->rateset.vht_mcsmap) ==
