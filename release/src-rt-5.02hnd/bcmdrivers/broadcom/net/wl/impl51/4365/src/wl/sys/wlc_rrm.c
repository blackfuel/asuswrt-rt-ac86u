...
#ifdef BCMDBG
#define WL_RRM(args)	do {if (wl_msg_level2 & WL_RRM_VAL) printf args;} while (0)
#else
#define	WL_RRM(args)
#endif /* BCMDBG */
	...
	dot11_rmreq_chanload_t *rmreq_chload, wlc_rrm_req_t *req)
{
	...
	WL_RRM(("<802.11K Rx Req> %s (%d bytes): token=%d reg=%d channel=0x%x dur=%d interval=%d\n",
		...
		case DOT11_RRM_STATS_GRP_ID_10:
			...
			if (delta) {
				...
				cur_stats = wlc_lq_chanim_chanspec_to_stats(wlc->chanim_info,
					wlc->chanspec); /* Use operating chanspec */
