...
wl_monitor(wl_info_t *wl, wl_rxsts_t *rxsts, void *p)
{
	...
	if (wl->monitor_type == 1) {
		...
		len = rtap_len + PKTLEN(wl->pub->osh, p) - D11_PHY_HDR_LEN;
		if ((mon_pkt = PKTGET(wl->pub->osh, len, FALSE)) == NULL)
			return;

		wl_radiotap_rx_p80211msg(wl->dev->name, rxsts,
			(p80211msg_t*)PKTDATA(wl->pub->osh, mon_pkt));
	}
		...
		mac_header = (struct dot11_header *)(PKTDATA(wl->pub->osh, p) + D11_PHY_HDR_LEN);

		if (rxsts->phytype != WL_RXS_PHY_N) {
			wl_radiotap_rx_legacy(mac_header, rxsts,
				(wl_radiotap_legacy_t*)PKTDATA(wl->pub->osh, mon_pkt));
		} else {
			wl_radiotap_rx_ht_brcm(mac_header, rxsts,
				(wl_radiotap_ht_brcm_t *)PKTDATA(wl->pub->osh, mon_pkt));
		}
	}
		...
		if (rxsts->phytype != WL_RXS_PHY_N || rxsts->encoding != WL_RXS_ENCODING_HT) {
			...
			wl_radiotap_rx_vht(mac_header, rxsts,
				(wl_radiotap_vht_t *)PKTDATA(wl->pub->osh, mon_pkt));
		}
#endif /* WL11AC */
		else if (rxsts->encoding == WL_RXS_ENCODING_HT) {
			wl_radiotap_rx_ht(mac_header, rxsts,
				(wl_radiotap_ht_t *)PKTDATA(wl->pub->osh, mon_pkt));
		}
	}
	else {
		return;
	...
	pdata = PKTDATA(wl->pub->osh, mon_pkt) + rtap_len;
	bcopy(PKTDATA(wl->pub->osh, p) + D11_PHY_HDR_LEN, pdata,
		PKTLEN(wl->pub->osh, p) - D11_PHY_HDR_LEN);
#else
