...
BCMATTACHFN(hnd_init)(void)
{
	...
	hnd_chipc_init(get_hnd_sih());

	/* Initialize CPU related stuff */
	hnd_cpu_init(get_hnd_sih());

	/* Initialize timer/time */
	hnd_timer_init(get_hnd_sih());

	/* Initialize deadman timer after hnd_timer_init */
	hnd_cpu_deadman_init(get_hnd_sih());

#ifdef RTE_CONS
	/* No printf's go to the UART earlier than this */
#ifndef BCM4350_FPGA
	(void)serial_init_devs(get_hnd_sih(), hnd_osh);
	(void)hnd_cons_init(get_hnd_sih(), hnd_osh);
#endif
#endif /* RTE_CONS */

#ifdef	SBPCI
	/* Init pci core if there is one */
	hnd_dev_init_pci(get_hnd_sih(), hnd_osh);
#endif

#ifdef BCMECICOEX
	/* Initialize ECI registers */
	hnd_eci_init(get_hnd_sih());
#endif

#ifdef WLGPIOHLR
	/* Initialize GPIO */
	rte_gpio_init(get_hnd_sih());
#endif
	hnd_gci_init(get_hnd_sih());
