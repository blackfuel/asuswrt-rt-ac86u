...
wlc_dump_stf(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "Enable sw_mask: %d sw_txchain 0X%x sw_rxchain 0x%x\n",
...
wlc_stf_chanspec_upd(wlc_info_t *wlc)
{
	...
	if (!WLCISACPHY(wlc->band))
		return;
#ifdef DYN160 /* TODO: REVISIT THIS BLOCK; IS THIS REQUIRED */
	if (DYN160_ACTIVE(wlc->pub) && WL_BW_CAP_160MHZ(wlc->band->bw_cap)) {
		int err;
		uint8 oper_mode = WL_OPER_MODE_INVALID, division = 1;
		if (wlc_modesw_init_max_rateset(wlc->modesw, 0) != BCME_OK) {
			WL_ERROR(("wl%d: %s: failed to init max rateset\n",
					wlc->pub->unit, __FUNCTION__));
		}
		if ((CHSPEC_IS8080(wlc->chanspec) || CHSPEC_IS160(wlc->chanspec))) {
			division = 2; // bw == 160/80p80 at half NSS
		}
		oper_mode = DOT11_OPER_MODE(0, WLC_BITSCNT(wlc->stf->hw_rxchain)/division,
				wlc_modesw_get_oper_bw(wlc->chanspec));
		if (oper_mode != WL_OPER_MODE_INVALID && wlc->cfg->oper_mode != oper_mode) {
			err = wlc_modesw_handle_oper_mode_notif_request(wlc->modesw, wlc->cfg,
					oper_mode, TRUE, MODESW_CTRL_OPMODE_IE_REQD_OVERRIDE);
			if (err != BCME_OK) {
				WL_ERROR(("wl%d: failed modesw %d\n", wlc->pub->unit, err));
			}
		}
	}
#endif /* DYN160 */
...
{
	wlc_info_t *wlc = (wlc_info_t *)hdl;
	wlc->stf->op_txstreams = wlc->stf->rxstreams;
	wlc->stf->op_rxstreams = wlc->stf->rxstreams;
	return BCME_OK;
}
