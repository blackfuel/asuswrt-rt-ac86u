			...
			if (BCM_DMA_CT_ENAB(wlc)) {
				...
				if ((queue != prev_queue) &&
					...
					if (wlc->cpbusy_war) {
						...
						dma_commit(WLC_HW_AQM_DI(wlc, prev_queue));
					}
					bulk_commit = FALSE;
				}
			}
...
wlc_txq_fill(txq_info_t *txqi, txq_t *txq)
{
	...
	for (ac = 0; ac < AC_COUNT; ac++) {
		added_time = txq->supply(txq->supply_ctx, ac, -1, &output_q, &fifo_idx);
...
wlc_prep_sdu_fast(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, struct scb *scb, void *sdu, uint *fifop,
	wlc_key_t *key, wlc_key_info_t *key_info)
{
	uint pktlen;
#if !defined(NEW_TXQ)
	uint ndesc;
...
#if defined(WLC_LOW) && defined(BCM_DMA_CT) && !defined(BCM_DMA_CT_DISABLED) && \
	defined(WL_MU_TX) && !defined(WL_MU_TX_DISABLED)
	/* Take care of high FIFOs if need */
	if (BCM_DMA_CT_ENAB(wlc)) {
		fifo_bitmap = BITMAP_SYNC_ALL_EXT_TX_FIFOS;
	}
	else
#endif
	fifo_bitmap = BITMAP_SYNC_ALL_TX_FIFOS;
		...
		for (swq = 0; swq < low_txq->nswq; swq++) {
			...
			while (pktq_ppeek(free_swq, swq) != head_pkt) {
				...
				if (PKTISTXFRAG(wlc->osh, pkt) &&
					...
					wlc_get_txh_info(wlc, pkt, &txh_info);
					fifo = WLC_TXFID_GET_QUEUE(ltoh16(txh_info.TxFrameID));
					WLC_TXFIFO_COMPLETE(wlc, fifo, 1, WLPKTTIME(pkt));
					PKTFREE(wlc->pub->osh, pkt, TRUE);
#if defined(WLAMPDU_MAC)
					flipEpoch |= TXQ_PKT_DEL;
#endif /* WLAMPDU_MAC */
