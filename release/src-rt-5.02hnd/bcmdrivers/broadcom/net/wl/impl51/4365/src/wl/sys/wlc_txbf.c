...
wlc_txbf_dump_stats(wlc_txbf_info_t *txbf, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "\t%s %d %s %d %s %d %s %d %s %d %s %d\n",
		...
		stats[2], wlc_read_shm(wlc, macstat1_shm_base + OFFSETOF(macstat1_t, txsf)),
		stats[3], wlc_read_shm(wlc, macstat1_shm_base + OFFSETOF(macstat1_t, txcwrts)),
		stats[4], wlc_read_shm(wlc, macstat1_shm_base + OFFSETOF(macstat1_t, txcwcts)),
		stats[5], wlc_read_shm(wlc, macstat1_shm_base + OFFSETOF(macstat1_t, txbfm)));
	bcm_bprintf(b, "\t%s %d %s %d %s %d %s %d %s %d\n",
		stats[6], wlc_read_shm(wlc, macstat1_shm_base + OFFSETOF(macstat1_t, rxndpaucast)),
		stats[7], wlc_read_shm(wlc, macstat1_shm_base + OFFSETOF(macstat1_t, bferptrdy)),
		stats[8], wlc_read_shm(wlc, macstat1_shm_base + OFFSETOF(macstat1_t, rxsfucast)),
		stats[9], wlc_read_shm(wlc, macstat1_shm_base + OFFSETOF(macstat1_t, rxcwrtsucast)),
		stats[10],
...
wlc_txbf_dump_shmx_bfi_blk(wlc_txbf_info_t *txbf, struct bcmstrbuf *b, uint8 idx)
{
	...
	bfi_blk = (txbf->mx_bfiblk_base + idx * BFI_BLK_SIZE) << 1;
	for (offset = 0; offset < BFI_BLK_SIZE; offset += 4) {
		if (b) {
			bcm_bprintf(b, "\toffset %2d: %04x %04x %04x %04x\n", offset,
			    wlc_read_shmx(wlc, bfi_blk + offset*2),
			    wlc_read_shmx(wlc, bfi_blk + (offset + 1)*2),
			    wlc_read_shmx(wlc, bfi_blk + (offset + 2)*2),
			    wlc_read_shmx(wlc, bfi_blk + (offset + 3)*2));
		} else {
			printf("\toffset %2d: %04x %04x %04x %04x\n", offset,
			    wlc_read_shmx(wlc, bfi_blk + offset*2),
			    wlc_read_shmx(wlc, bfi_blk + (offset + 1)*2),
