 ...
 *
 * $Id: wlc_tdls.c 642012 2016-06-07 04:54:41Z $
 */
...
wlc_tdls_cal_mic_chk(tdls_info_t *tdls, struct scb *scb, uchar *tlvs, int tlvs_len, uint8 transId)
{
	...
	bcm_tlv_t *ie;
	uint data_len = 0;
	uchar *data = NULL;	/* TK_bits + 128, where TK_bits is 16 bytes for CCMP */
	uint8 *mic_save = NULL;
	bool ret = FALSE;

	if ((data = MALLOC(tdls->osh, 256)) == NULL)
		goto fail;

	if ((mic_save = MALLOC(tdls->osh, EAPOL_WPA_KEY_MIC_LEN)) == NULL)
		goto fail;

	linkId_ie = (link_id_ie_t*)bcm_parse_tlvs(tlvs, tlvs_len, DOT11_MNG_LINK_IDENTIFIER_ID);
	if (!linkId_ie) {
		WL_ERROR(("wl%d:%s(): no Link Identifier IE!\n",
			tdls->pub->unit, __FUNCTION__));
		goto fail;
	}
	if (linkId_ie->len != TDLS_LINK_ID_IE_LEN) {
	...
	data_len = 0;
	bcopy((uint8 *)&linkId_ie->tdls_init_mac, (char *)&data[data_len], ETHER_ADDR_LEN);
	data_len += ETHER_ADDR_LEN;
	/* TDLS responder STA MAC addr */
	bcopy((uint8 *)&linkId_ie->tdls_resp_mac, (char *)&data[data_len], ETHER_ADDR_LEN);
	data_len += ETHER_ADDR_LEN;
	/* Transaction Sequence number */
	data[data_len] = transId;
	data_len++;
	/* Link Identifier IE */
	bcopy((char*)linkId_ie, &data[data_len], (TDLS_LINK_ID_IE_LEN + TLV_HDR_LEN));
	data_len += (TDLS_LINK_ID_IE_LEN + TLV_HDR_LEN);
	/* RSN IE */
	ie = bcm_parse_tlvs(tlvs, tlvs_len, DOT11_MNG_RSN_ID);
	if (!ie) {
		WL_ERROR(("wl%d:%s(): no RSN IE!\n",
			tdls->pub->unit, __FUNCTION__));
		goto fail;
	}

	if (data_len + ie->len + TLV_HDR_LEN >= 256) {
		WL_ERROR(("wl%d:%s(): data[] buffer over-run!\n",
			tdls->pub->unit, __FUNCTION__));
		goto fail;
	}
	bcopy((char*)ie, &data[data_len], (ie->len + TLV_HDR_LEN));
	data_len += (ie->len + TLV_HDR_LEN);
	/* Timeout Interval IE */
	ie = bcm_parse_tlvs(tlvs, tlvs_len, DOT11_MNG_FT_TI_ID);
	if (!ie) {
		WL_ERROR(("wl%d:%s(): no Timeout Interval IE!\n",
			tdls->pub->unit, __FUNCTION__));
		goto fail;
	}
	if (ie->len != sizeof(ti_ie_t)) {
		WL_ERROR(("wl%d:%s(): wrong Timeout Interval IE len = %d!\n",
	...
	if (!ie) {
		...
		if (ie->len != sizeof(dot11_ft_ie_t) - TLV_HDR_LEN) {
			WL_ERROR(("wl%d:%s(): wrong FT IE len = %d!\n",
				tdls->pub->unit, __FUNCTION__, ie->len));
			goto fail;
		}
		...
		if (memcmp(ft_ie->mic, mic_save, EAPOL_WPA_KEY_MIC_LEN)) {
			bcopy(ft_ie->mic, mic_save, EAPOL_WPA_KEY_MIC_LEN);
			check_mic = TRUE;
		}
	...
	TDLS_PRHEX("data:", data, data_len);
	aes_cmac_calc(data, data_len, scb_tdls->tpk, EAPOL_WPA_KEY_MIC_LEN, scb_tdls->mic,
		EAPOL_WPA_KEY_MIC_LEN);
	TDLS_PRHEX("mic:", scb_tdls->mic, EAPOL_WPA_KEY_MIC_LEN);
	...
	TDLS_PRHEX("MIC:", scb_tdls->mic, 16);
	ret = TRUE;

fail:
	if (mic_save)
		MFREE(tdls->osh, mic_save, EAPOL_WPA_KEY_MIC_LEN);
	if (data)
		MFREE(tdls->osh, data, 256);
	...
	uint16 reason, bool check_mic)
{
	...
	if ((data = MALLOC(tdls->osh, 256)) == NULL)
		goto fail;
	if (check_mic &&
		(mic_save = MALLOC(tdls->osh, EAPOL_WPA_KEY_MIC_LEN)) == NULL)
		goto fail;
	...
	/* reason code */
	*((uint16*)&data[data_len]) = reason;
	data_len += 2;

	/* token */
	data[data_len] = scb_tdls->con_token;
	data_len++;
	...
	if (check_mic) {
		bcopy(((dot11_ft_ie_t*)ie)->mic, mic_save, EAPOL_WPA_KEY_MIC_LEN);
		bzero(((dot11_ft_ie_t*)ie)->mic, EAPOL_WPA_KEY_MIC_LEN);
	}
	bcopy((char*)ie, &data[data_len], (ie->len + TLV_HDR_LEN));
	data_len += (ie->len + TLV_HDR_LEN);
	TDLS_PRHEX("data:", data, data_len);
	aes_cmac_calc(data, data_len, scb_tdls->tpk, EAPOL_WPA_KEY_MIC_LEN,
	...
	uint offset)
{
	...
	if ((p = wlc_frame_get_mgmt(wlc, FC_ACTION, f->sa, &parent->cur_etheraddr,
	                            &parent->BSSID, TDLS_ACTION_FRAME_DEFAULT_LEN, &data))
		== NULL) {
		return BCME_NOMEM;
	}
