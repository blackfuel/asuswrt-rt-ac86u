	...
	if (CHSPEC_IS2G(chanspec)) {
		...
		if (CHSPEC_IS20(chanspec)) {
			...
			wlc_phy_txpwr_srom13_ext_1024qam_convert_mcs_5g(
				...
				chanspec, tmp_max_pwr, &mcs40_offset_5g);
			...
			if (CHSPEC_IS40(chanspec)) {
				...
				wlc_phy_txpwr_srom13_ext_1024qam_convert_mcs_5g(
					sr13->pp1024qam5g[band5g],
					chanspec, tmp_max_pwr, &mcs80_offset_5g);
				...
				if (CHSPEC_IS80(chanspec)) {
					...
					wlc_phy_txpwr_srom13_ext_1024qam_convert_mcs_5g(
						sr13->pp1024qam5g[band5g],
						chanspec, tmp_max_pwr, &mcs160_offset_5g);
...
BCMATTACHFN(wlc_phy_txpwr_srom13_read_ppr)(phy_info_t *pi)
{
	...
	if (!(SROMREV(pi->sh->sromrev) < 13)) {
	    ...
	    pi->ppr.sr13.offset_2g = (((nibble<<8)|(nibble<<12))|(nibble01))&0xffff;
	    /* 2G OFDM_20 */
	    pi->ppr.sr13.ofdm_2g.bw20	= (uint16)PHY_GETINTVAR(pi, rstr_dot11agofdmhrbw202gpo);
	    /* 2G MCS_20 */
	...
	int alen, int vsize)
{
	...
	case IOV_GVAL(IOV_PHY_MUTED):
		*ret_int_ptr = PHY_MUTED(pi) ? 1 : 0;
		break;
#ifdef WL_MU_TX
	case IOV_GVAL(IOV_PHY_MU_NSS4ENABLE):
		if (ISACPHY(pi) && ACMAJORREV_33(pi->pubpi.phy_rev))
			wlc_phy_mu_get_nss4enable(pi, ret_int_ptr);
		else
			err = BCME_UNSUPPORTED;
		break;

	case IOV_SVAL(IOV_PHY_MU_NSS4ENABLE):
		if (ISACPHY(pi) && ACMAJORREV_33(pi->pubpi.phy_rev)) {
			if ((int_val == 0) || (int_val == 1))
				wlc_phy_mu_set_nss4enable(pi, int_val);
			else
				err = BCME_RANGE;
		} else {
			err = BCME_UNSUPPORTED;
...
#ifdef WL_MU_TX
void
wlc_phy_mu_get_nss4enable(phy_info_t *pi, int *ret_int_ptr)
{
	int mem_id = 4;
	int svmp_addr = 0x2b;
	bool suspend;
	...
	wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_SVMPMEMS, 1, 0x8000, 32, &mem_id);
	wlc_phy_table_read_acphy(pi, ACPHY_TBL_ID_SVMPMEMS, 1, svmp_addr, 32, &tmp_val);

	*ret_int_ptr = tmp_val & 0xffff;
	/* restore stall value */
	ACPHY_ENABLE_STALL(pi, stall_val);
...
void
wlc_phy_mu_set_nss4enable(phy_info_t *pi, int int_val)
{
	int mem_id = 4;
	int svmp_addr = 0x2b;
	bool suspend;
	...
	wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_SVMPMEMS, 1, 0x8000, 32, &mem_id);
	wlc_phy_table_read_acphy(pi, ACPHY_TBL_ID_SVMPMEMS, 1, svmp_addr, 32, &tmp_val);
	tmp_val = (tmp_val & 0xffff0000) | ((uint32) int_val);
	wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_SVMPMEMS, 1, svmp_addr, 32, &tmp_val);
