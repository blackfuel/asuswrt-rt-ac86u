...
wlc_bsscfg_t * bsscfg)
{
	...
	if (nss_change) {
		 ...
		 */
		wlc->stf->op_txstreams = wlc->stf->op_rxstreams = rxnss;
	...
	uint8 oper_mode, uint8 enabled)
{
	...
	if (!wlc_modesw_is_downgrade(wlc, bsscfg, bsscfg->oper_mode, oper_mode)) {
		...
		if ((old_rxnss != new_rxnss) || (new_bw != old_bw)) {
	...
	wlc_bsscfg_t *bsscfg, uint8 oper_mode, uint8 enabled, uint32 ctrl_flags)
{
	...
	if (((new_bw < old_bw) && (new_nss > old_nss)) ||
		...
		WL_MODE_SWITCH(("Rejecting new bw: %d, old bw: %d; new nss: %d, old nss: %d\n",
				new_bw, old_bw, new_nss, old_nss));
		return BCME_BADARG;
	}

	/* TODO: ADD 4366C0 specific checks here */
	if ((new_bw == old_bw) && WLC_PHY_AS_80P80(wlc, bsscfg->current_bss->chanspec) &&
		(new_nss > 2)) {
		WL_MODE_SWITCH(("Reject Oper mode update for 160Mhz as nss > 2 \n"));
		return BCME_BADARG;
	}

	WL_MODE_SWITCH(("Processing new bw: %d, old bw: %d; new nss: %d, old nss: %d\n",
			new_bw, old_bw, new_nss, old_nss));
...
wlc_modesw_get_oper_bw(chanspec_t chanspec)
{
	...
	switch (ch_bw) {
		case WL_CHANSPEC_BW_160:
			return DOT11_OPER_MODE_160MHZ;
		case WL_CHANSPEC_BW_8080:
			return DOT11_OPER_MODE_8080MHZ;
...
{
	wlc_info_t *wlc = modesw_info->wlc;
	uint8 bw = DOT11_OPER_MODE_20MHZ, rxnss = 0, rxnss_type, oper_mode;
	...
	if (wlc_modesw_is_connection_vht(wlc, bsscfg)) {
		...
		oper_mode = DOT11_OPER_MODE(rxnss_type, rxnss, bw);
	} else {
		oper_mode = bw;
	}
...
wlc_modesw_mutx_state_upd_cb(void *ctx, mutx_state_upd_data_t *notif_data)
{
	...
	FOREACH_UP_AP(wlc, idx, bsscfg)
	{
		...
		new_oper_mode = DOT11_OPER_MODE(0, nss, DOT11_OPER_MODE_80MHZ);

		/* Check if current chanpsec bandwidth is 160MHz/80p80 */
		if (curr_oper_mode == DOT11_OPER_MODE(0, nss, DOT11_OPER_MODE_160MHZ)) {
