...
wlc_mutx_dump(wlc_mutx_info_t *mu_info, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "Total mutx: queued %u tx_as_mu %u nlost_mu %u nlost_su %u\n",
...
wlc_mutx_txfifo_enable_filter(wlc_info_t *wlc, uint fifo)
{
	...
	if (BUSTYPE(wlc->pub->sih->bustype) == SI_BUS && (fifo >= TX_FIFO_MU_START) &&
		...
		(D11REV_IS(wlc->pub->corerev, 65) && (fifo > TX_FIFO_19)))) {
		/* Skip this FIFO */
		return FALSE;
	}
...
wlc_mutx_hostflags_update(wlc_info_t *wlc)
{
	...
	mxhf_mask = (MXHF0_CHKFID | MXHF0_DYNSND | MXHF0_MUAGGOPT);
	...
	if (MU_TX_ENAB(wlc)) {
		...
		mxhf_val = (MXHF0_CHKFID | MXHF0_DYNSND | MXHF0_MUAGGOPT);
	} else {
