...
void parse_cpu_major_mode(int argc, char **argv)
{
    ...
    if (do_help)
    {
        ...
        printf("   --bind_to_cpu (-b): bind thread to a particular CPU, values are 0 or 1, -1 means no binding\n");
    ...
    if (bind_to_cpu < -1 || bind_to_cpu > 1) {
        printf("----error: bind_to_cpu must be 0 or 1\n");
        exit(-1);
    }
