...
REBUILD_WL_MODULE=$(shell if [ -d "$(WLSRC_BASE)/wl/sys" -a "$(REUSE_PREBUILT_WL)" != "1" ]; then echo 1; else echo 0; fi)
endif
...
ifeq ($(REBUILD_WL_MODULE),1)
    ...
    include $(WLSRC_BASE)/router/.config

    KBUILD_CFLAGS += -I$(WLSRC_BASE)/include
    KBUILD_CFLAGS += $(WLAN_ComponentIncPath)
    KBUILD_CFLAGS += $(WLAN_StdIncPathA)
    KBUILD_CFLAGS += -I$(WLSRC_BASE)/common/include
    KBUILD_AFLAGS += -I$(WLSRC_BASE)/include
    KBUILD_AFLAGS += -I$(WLSRC_BASE)/common/include
    KBUILD_CFLAGS += -DBCMDRIVER -Dlinux
    KBUILD_CFLAGS += -DBCA_HNDROUTER -Wno-error=date-time
...
ifeq ($(ARCH), arm)
    ...
    ifeq ($(WLCLMAPI),1)
    CLM_TYPE ?= router
    $(call WLAN_GenClmCompilerRule,$(WLSRC_BASE)/wl/clm/src,$(WLSRC_BASE))
    endif
    ...
    ifneq ($(findstring PHY_HAL,$(WLFLAGS)),)
        EXTRA_CFLAGS += -I$(WLSRC_BASE)/wl/phy
    else
